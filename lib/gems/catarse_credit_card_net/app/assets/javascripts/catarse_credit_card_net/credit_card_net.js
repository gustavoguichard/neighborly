    ...
    if(this.card_ok()) {
      ...
      $.post($('form#credit_card_form').prop('action'), card_data, function(response){
        if(response.process_status == 'ok') {
          ...
          $('.cardError p.errorName').text(response.message);
          $('.cardError').fadeIn(300);
          $('form#credit_card_form .submit input').removeClass('disabled');
          $('form#credit_card_form .submit input').val('Confirm');
        }
      }, 'json').error(function(){
        $('.cardError p.errorName').text('The back already confirmed, if you can do another back please refresh the page or go back to project page :)');
      $('.cardError').fadeIn(300);
      });
    }
  },
