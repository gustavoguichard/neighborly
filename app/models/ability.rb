...
class Ability
  ...
  def initialize(current_user, options = {})
    ...
    can :update, :projects, [:about, :video_url, :background, :uploaded_image, :hero_image, :headline, :budget, :terms, :address_neighborhood, :address, :address_city, :address_state, :hash_tag, :site, :tag_list] do |project|
      (project.user == current_user || ( project.last_channel.user == current_user rescue false )) && ( project.online? || project.waiting_funds? || project.successful? || project.failed? )
    end

    can :update, :projects do |project|
      (project.user == current_user || ( project.last_channel.user == current_user rescue false )) && ( project.draft? || project.soon? || project.rejected? || project.in_analysis? )
    end
